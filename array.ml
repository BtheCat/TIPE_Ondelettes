(* A voir : fonction create pour le type Matrix (ligne 185) *)
...
let moyenne a b sum prod inv one =
	let two = sum one one in
	prod (sum a b) (inv two)
...
module type VECT =
	sig
		...
		val ceil : int -> int -> int
		val length : 'a vect -> int
		val create : 'a -> int -> 'a vect
		val make : (int -> 'a) -> int -> 'a vect
		val create_empty : 'a -> 'a vect
		val duplicate : 'a vect -> int -> 'a vect array
		val make_matrix : 'a -> int -> int -> 'a vect vect 
		val create_tab_ind : int -> int -> int vect
		...
		val sum_scal : 'a vect -> 'a -> ('a -> 'a -> 'a) -> 'a vect
		val prod_scal : 'a vect -> 'a -> ('a -> 'a -> 'a) -> 'a vect
		...
		val copy : 'a vect -> int array -> 'a vect

		val filter : 'a vect -> 'a vect -> 'a vect -> ('a -> 'a -> 'a) -> ('a -> 'a -> 'a) -> ('a -> 'a) -> 'a -> 'a vect
		
		val print_str : string -> unit
		val max_length_coef : 'a vect -> ('a -> int) -> int
		val max_abs : 'a vect -> ('a -> 'a) -> 'a
		val show : 'a vect -> ('a -> unit) -> ('a -> int) -> int -> unit
	end;;
...
module Vect : VECT =
	struct
		...
		let create_tab_ind ind_beg ind_end =
			let v = create ind_beg (ind_end - ind_beg + 1) in
			for i = 1 to ind_end - ind_beg do
				affect v (ind_beg + i) i
			done; v
		...
		let sum_scal v scal sum_elem = sum v (create scal (length v)) sum_elem

		let prod_scal v scal prod_elem = prod v (create scal (length v)) prod_elem
		...
		let copy v tab_indices =
			let tab_ind = (if tab_indices <> [||] then tab_indices
				else range (length v) ) in
			let n = Array.length tab_ind in
			let v_copy = create ( value v tab_ind.(0) ) n in
			for i = 1 to (Array.length tab_ind - 1) do
				affect v_copy (value v tab_ind.(i)) i
			done; v_copy

		let duplicate v n = 
			let tab_ind_all = Array.init (length v) (function x -> x) in
			let t = Array.make n (create_empty (value v 0)) in
			for i = 0 to n - 1 do
				t.(i) <- copy v tab_ind_all
			done;
			t
		...
		let filter b a x sum prod inv zero =
			...
			let a = (if n > na then extend a (n - na) zero else a)
			and b = (if n > nb then extend b (n - nb) zero else b) in	
		...
		let max_length_coef vect len =
                        let max = ref 0 in
                        let size = length vect in
                        for i = 0 to size - 1 do
                                let l = len (value vect i) in
                                if l > !max then max := l
                       done; !max
		...
		let max_abs vect abs =
			let maxi = ref (abs (value vect 0)) in
			for i = 1 to (length vect - 1) do
				let value_i = abs (value vect i) in
				if value_i > !maxi then
					maxi := value_i
			done;
			!maxi

                let show vect print len len_max =
                        let size = length vect in
                        let len_coef_max = 
				if len_max = 0 then max_length_coef vect len
				else len_max
			in
                        for i = 0 to size - 1 do
                                let v = value vect i in
				let delta_len = len_coef_max - (len v) in
				print_str (String.make (ceil delta_len 2) ' ');
                                print v; print_char ' ';
                                print_str (String.make (delta_len / 2) ' ')
                        done;
                        print_char '\n'
...
module type MATRIX =
	sig
		...
		val join : 'a Vect.vect array -> 'a matrix
		val dim : 'a matrix -> (int * int)
		val create : 'a -> int -> int -> 'a matrix
		val duplicate : 'a Vect.vect -> int -> 'a matrix
		val matrix_of_vect : 'a Vect.vect -> 'a matrix
		val arrays_of_matrix : 'a matrix -> 'a array array
		val matrix_of_arrays : 'a array array -> 'a matrix
		val identity : int -> 'a -> 'a -> 'a matrix
		...
		val swap_lines : 'a matrix -> (int * int) -> unit
		val swap_columns : 'a matrix -> (int * int) -> unit
		val line : 'a matrix -> int -> 'a Vect.vect
		val affect_line : 'a matrix -> 'a Vect.vect -> (int * int) -> unit

		val pow : 'a -> int -> ('a -> 'a -> 'a) -> 'a -> 'a

		val sum : 'a matrix -> 'a matrix -> ('a -> 'a -> 'a) -> 'a matrix
		val prod : 'a matrix -> 'a matrix -> ('a -> 'a -> 'a) -> 'a -> ('a -> 'a -> 'a) -> 'a matrix
		val transpo : 'a matrix -> 'a matrix
		val trace : 'a matrix -> ('a -> 'a -> 'a) -> 'a -> 'a
		val prod_scal_cano : 'a matrix -> 'a matrix -> ('a -> 'a -> 'a) -> ('a -> 'a -> 'a) -> 'a -> 'a
		val norm_eucli : 'a matrix -> ('a -> 'a -> 'a) -> ('a -> 'a -> 'a) -> 'a -> 'a
		val norm_sup : 'a matrix -> ('a -> 'a) -> 'a

		val extr_matrix : 'a matrix -> (int * int) -> (int * int) -> 'a matrix
		val dilate : 'a matrix -> int -> ('a -> 'a -> 'a) -> ('a -> 'a -> 'a) -> ('a -> 'a) -> 'a -> 'a matrix
		
		val max_length_coef : 'a matrix -> ('a -> int) -> int
		val show : 'a matrix -> ('a -> unit) -> ('a -> int) -> unit
	end;;
...
module Matrix : MATRIX =
	struct
		...
		let join vect_array =
			let n = Array.length vect_array in
			let mat = Vect.create (vect_array.(0)) n in
			for i = 1 to n - 1 do
				Vect.affect mat (vect_array.(i)) i
			done; mat
		...
		let duplicate v n = join (Vect.duplicate v n)

		let matrix_of_vect v = Vect.create v 1
		
		let arrays_of_matrix mat =
			let n = fst (dim mat) in
			let t = Array.make n (Vect.array_of_vect (Vect.value mat 0)) in
			for i = 1 to n - 1 do
				t.(i) <- Vect.array_of_vect (Vect.value mat i)
			done; t

		let matrix_of_arrays t =
			let n = Array.length t in
			let mat = create (t.(0).(0)) n n in
			for i = 0 to n - 1 do
				Vect.affect mat (Vect.vect_of_array t.(i)) i
			done; mat

		let identity n un zero =
			let mat = create zero n n in
			for i = 0 to n - 1 do
				affect mat un (i, i)
			done; mat
		...
		let rec pow x n prod_elem un =
			if n = 0 then un
			else 
				let y = pow x (n / 2) prod_elem un in
				if n mod 2 = 0 then
					prod_elem y y
				else
					prod_elem x (prod_elem y y)
		
		let max_length_coef mat len =
			let max = ref 0 in
			let size = dim mat in
			for i = 0 to fst size - 1 do
				let max_line = Vect.max_length_coef (line mat i) len in
				if max_line > !max then
					max := max_line
			done; !max

		let show mat print len =
			let size = dim mat in
			let len_coef_max = max_length_coef mat len in

			for i = 0 to fst size - 1 do			
				for j = 0 to snd size - 1 do
					let v = value mat (i, j) in
					let delta_len = len_coef_max - (len v) in
					Vect.print_str (String.make (Vect.ceil delta_len 2) ' ');
					print v; print_char ' ';
					Vect.print_str (String.make (delta_len / 2) ' ')
				done;
				print_char '\n'
			done
		...
		let prod mat1 mat2 prod_elem zero sum_elem =
			let size = dim mat1 in
			let size2 = dim mat2 in
			if not (fst size = snd size2 && snd size = fst size2) then
				failwith "Matrix.prod"
			else
				let mat = create (value mat1 (0, 0)) (fst size) (snd size2) in
				for i = 0 to fst size - 1 do
					for j = 0 to snd size2 - 1 do
						let s = ref zero in
						for k = 0 to snd size - 1 do
							s := sum_elem !s (prod_elem (value mat1 (i, k)) (value mat2 (k, j)))
						done;
						affect mat !s (i, j)
					done
				done; mat
		...
		let transpo mat =
			let (n, m) = dim mat in
			let mat_transpo = create (value mat (0, 0)) m n in
			for i = 0 to n - 1 do
				for j = 0 to m - 1 do
					affect mat_transpo (value mat (i, j)) (j, i)
				done
			done; mat_transpo

		let trace mat sum_elem zero =
			let n, m = dim mat in
			if not (n = m) then
				failwith "Matrix.trace"
			else
				let tr = ref zero in
				for i = 0 to n - 1 do
					tr := sum_elem !tr (value mat (i, i))
				done; !tr
		...
		let norm_sup mat abs =
			let maxi = ref (value mat (0, 0)) in
			for i = 0 to (Vect.length mat - 1) do
				let max_line = Vect.max_abs (line mat i) abs in
				if max_line > !maxi then
					maxi := max_line
			done;
			!maxi

		let swap_lines mat (i, j) = 
			let vi = Vect.copy (line mat i) [||] in
			affect_line mat (line mat j) (i, 0);
			affect_line mat vi (j, 0)

		let swap_columns mat (i, j) =
			let mat_transpo = transpo mat in
			swap_lines mat_transpo (i, j);
			put_in mat (transpo mat_transpo) (0, 0)
		...
		let dilate mat dil_fact sum prod inv one =
			let (n, m) = dim mat in
			let dil_mat = create (value mat (0, 0)) (dil_fact * n) (dil_fact * m) in
			for i = 0 to n - 1 do
				for j = 0 to m - 1 do
					affect dil_mat (value mat (i, j)) (dil_fact * i, dil_fact * j);
					for l = 1 to dil_fact - 1 do
						if j <> m - 1 then
							affect dil_mat (moyenne (value mat (i, j)) (value mat (i, j + 1)) sum prod inv one ) 
									(dil_fact * i + l, dil_fact * j + l)
						else
							affect dil_mat (value mat (i, j)) (dil_fact * i + l, dil_fact * j + l)
					done
				done
			done;
			dil_mat
...
module type PIXEL_MATRIX =
	sig
		...
		val arrays_of_pixmat : pixel_matrix -> pixel array array
		...
		val show_me : pixel_matrix -> unit
		
		val negative : pixel_matrix -> pixel_matrix
		...
		val mirror_left_right : pixel_matrix -> pixel_matrix
		val mirror_up_down : pixel_matrix -> pixel_matrix
	end;;
...
module Pixel_matrix : PIXEL_MATRIX =
	struct
		...
		let arrays_of_pixmat pixmat =
			let (n, m) = Matrix.dim pixmat in
			let t = Array.make_matrix n m (0, 0, 0) in
			for i = 0 to n - 1 do
				for j = 0 to m - 1 do
					t.(i).(j) <- Matrix.value pixmat (i, j)
				done
			done;
			t
		...
		let negative m =
			let (w, h) = Matrix.dim m in
			let m1 = Matrix.create (0, 0, 0) w h in
			for i = 0 to w - 1 do
				for j = 0 to h - 1 do
					let (r, g, b) = Matrix.value m (i, j) in
					Matrix.affect m1 (255 - r, 255 - g, 255 - b) (i, j)
				done
			done;
			m1
		...
		let mirror_left_right m =
			...
			for j = 0 to h - 1 do
				for i = 0 to (w - 1) / 2 do
					Matrix.affect m1 (Matrix.value m (w - i - 1, j)) (i, j);
					Matrix.affect m1 (Matrix.value m (i, j)) (w - i - 1, j)
				done
			done;
			m1
		...
		let mirror_up_down m =
			...
			for i = 0 to w - 1 do
				for j = 0 to (h - 1) / 2 do
					Matrix.affect m1 (Matrix.value m (i, h - j - 1)) (i, j);
					Matrix.affect m1 (Matrix.value m (i, j)) (i, h - j - 1)
				done
			done;
			m1
